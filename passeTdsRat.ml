...
| AstSyntax.New ty -> New ty
| AstSyntax.Adresse s -> (match chercherGlobalement tds s with
                          |Some i -> (match info_ast_to_info i with
                                      | InfoConst (_,_) -> raise (MauvaiseUtilisationIdentifiant s)
                                      | InfoVar (_,_,_,_) -> Adresse i
                                      | InfoFun (_,_,_) -> raise (MauvaiseUtilisationIdentifiant s))
                          |None -> raise (IdentifiantNonDeclare s))
| AstSyntax.Null -> Null
...
let rec analyse_tds_instruction tds oia i =
  ...
  | AstSyntax.Affectation (n,e) -> Affectation ((analyse_tds_affectable tds true n),(analyse_tds_expression tds e))
